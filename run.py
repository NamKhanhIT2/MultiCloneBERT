...
def evaluate(args, model, tokenizer, eval_when_training=False):
    ...
    accuracy = (y_preds == y_trues).mean()
    
    # Calculate accuracy for each class
    unique_labels = np.unique(y_trues)
    class_accuracies = []
    for label in unique_labels:
        class_mask = y_trues == label
        class_accuracy = (y_preds[class_mask] == y_trues[class_mask]).mean()
    ...
    result = {
        ...
        "eval_macro_accuracy": float(macro_accuracy),
    ...
    output_path = output_dir / "confusion_matrix.svg"
...
def test(args, model, tokenizer, best_threshold=0):
    ...
    with open(os.path.join(args.output_dir, "predictions.txt"), 'w') as f:
        for example, pred in zip(eval_dataset.examples, y_preds):
            f.write(example.url1 + '\t' + example.url2 +
                    '\t' + str(pred) + '\n')
